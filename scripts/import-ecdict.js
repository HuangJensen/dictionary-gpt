...
// 流式导入：逐行读取 + 批量插入，内存占用稳定
async function importFromFile(conn, file, opts = {}) {
...
async function main() {
  ...
  const { total, skipped } = await importFromFile(conn, file, { reset: reset });
  console.log('导入完成: 共 ' + total + ' 条，跳过 ' + skipped + ' 条');
  await conn.end();
}

if (require.main === module) {
  main().catch(function (err) { console.error("导入失败:", err.message); process.exit(1); });
}

module.exports = { parseCSVLine: parseCSVLine, importFromFile: importFromFile };
